...
function WorkspaceItemList({workspaceList}) {
    ...
    const OnClickWorkspaceItem = (workspaceId, workspaceName) => {
        router.push('/workspace/' + workspaceId + '?space_name=' + encodeURIComponent(workspaceName));
      }
  ...
  return (
    <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-6'>
        {workspaceList&&workspaceList.map((workspace,index)=>(
            ...
            onClick={() => OnClickWorkspaceItem(workspace.id, workspace.workspaceName)}
            >
