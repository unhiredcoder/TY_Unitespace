...
import React, { useState } from 'react';
import uuid4 from 'uuid4';
import coverImages from "@/app/_shared/CoverOption.js"
...
function CreateWorkspace() {
    ...
    const getRandomCoverImage = () => {
        ...
        return coverImages[randomIndex].imageUrl;
    };
    const [coverImage, setCoverImage] = useState(getRandomCoverImage());
    const [workspaceName, setWorkspaceName] = useState('');
    const [emoji, setEmoji] = useState(''); // Emoji state can be an empty string
    const { user } = useUser();
    const { orgId } = useAuth();
    const [loading, setLoading] = useState(false);
    const router = useRouter();
                ...
                <div className='p-12'>
                    ...
                    <div className='mt-7 flex justify-end gap-6'>
                        ...
                        >
                            {loading ? (
                                <>
                                    Please wait <Loader2Icon className='animate-spin ml-2' />
                                </>
                            ) : (
                                'Create'
                            )}
